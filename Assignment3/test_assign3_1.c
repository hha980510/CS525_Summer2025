...
Record *testRecord(Schema *schema, int a, char *b, int c)
{
	...
	RC rc = createRecord(&result, schema);
	if (rc != RC_OK || result == NULL || result->data == NULL) {
		printf("createRecord failed! RC=%d, result=%p, result->data=%p\n", rc, result, result ? result->data : NULL);
		exit(1);
	}
...
int main(void) {
    ...
    printf("DONE testInsertManyRecords\n"); fflush(stdout);

    testRecords();
    printf("DONE testRecords\n"); fflush(stdout);

    testCreateTableAndInsert();
    printf("DONE testCreateTableAndInsert\n"); fflush(stdout);

    testUpdateTable();
    printf("DONE testUpdateTable\n"); fflush(stdout);

    testScans();
    printf("DONE testScans\n"); fflush(stdout);

    testScansTwo();
    printf("DONE testScansTwo\n"); fflush(stdout);

    testMultipleScans();
    printf("DONE testMultipleScans\n"); fflush(stdout);
...
testCreateTableAndInsert (void)
{
	...
	for(i = 0; i < numInserts; i++)
	{
		...
		printf(">> calling insertRecord at i = %d\n", i); fflush(stdout);
		rids[i] = r->id;
	}
...
{
    printf("==> START testInsertManyRecords\n"); fflush(stdout);

	RM_TableData *table = (RM_TableData *) malloc(sizeof(RM_TableData));
	TestRecord inserts[] = {
		{1, "aaaa", 3}, {2, "bbbb", 2}, {3, "cccc", 1},
		{4, "dddd", 3}, {5, "eeee", 5}, {6, "ffff", 1},
		{7, "gggg", 3}, {8, "hhhh", 3}, {9, "iiii", 2}, {10, "jjjj", 5},
	};
	...
	printf("Inserting %d records...\n", numInserts); fflush(stdout);
	for(i = 0; i < numInserts; i++)
	{
		printf(">> Insert loop i = %d\n", i); fflush(stdout);
		...
		if (r == NULL || r->data == NULL) {
			printf(">> ERROR: fromTestRecord returned NULL at i = %d\n", i); fflush(stdout);
			exit(1);
		}

		RC rc = insertRecord(table, r);
		if (rc != RC_OK) {
			printf(">> ERROR: insertRecord failed at i = %d with code %d\n", i, rc); fflush(stdout);
			exit(1);
		}

		printf(">> Successfully inserted record at i = %d\n", i); fflush(stdout);
	...
	printf("Closing and re-opening table...\n"); fflush(stdout);
	TEST_CHECK(closeTable(table));
	TEST_CHECK(openTable(table, "test_table_t"));

	printf("Verifying inserted records...\n"); fflush(stdout);
	for(i = 0; i < numInserts; i++)
	{
		RID rid = rids[i];
